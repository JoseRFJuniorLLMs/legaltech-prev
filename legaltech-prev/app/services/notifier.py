...
class Notifier:
    def _send(self, to: str, subject: str, body: str, attachment: Optional[Path] = None) -> Dict[str, Any]:
    ...
    def send_link(self, email: str, link: str, whatsapp: Optional[str] = None) -> Dict[str, Any]:
        body = (
        ...
        res = self._send(email, "Seu link para enviar o CNIS", body)
        if whatsapp:
            logging.info("[WHATSAPP fase 2] enviaria link para %s: %s", whatsapp, link)
        return res

    def send_peticao(self, lead: Dict[str, Any], peticao_path: Optional[Path]) -> Dict[str, Any]:
        body = (
